...
\Fact{oursUpdate}\,ns and answers a historical query in \Fact{oursQuery}\,ns; at one
million elements, \Fact{oursUpdateBig}\,ns and \Fact{oursQueryBig}\,ns. Publishing
becomes \Fact{oursGrowth}$\times$ more expensive while the array grows a
thousandfold. The tree's height only doubles over that span and the non-persistent
...
This work is flat over the same span --- \Fact{widthPlateauLow}\,ns to
\Fact{widthPlateauHigh}\,ns across four orders of magnitude --- because a fully
...
costs \Fact{widthWholeArray}\,ns. \emph{The widest update in the sweep is by a wide
...
this work answers in \Fact{auditOldestOurs}\,ns and checkpoint-and-replay costs
...
\Fact{oursTailP50}\,ns and the slowest one in a hundred \Fact{oursTailP99}\,ns. The
extreme values --- up to \Fact{oursTailMax}\,ns --- are the \texttt{std::vector}
...
\midrule
Array size & cheaper up to $\approx10^5$ elements & cheaper at $10^6$, by
\Fact{marginAtMillionUpdate}$\times$ on updates and
\Fact{marginAtMillionQuery}$\times$ on queries \\
