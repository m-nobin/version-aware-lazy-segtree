...
| C3 frontier laws and lower bound | DSST/MVBT optimality for point updates; Fiat-Kaplan confluent lower bounds | Frontier laws drafted, review pending; lower bound withdrawn. `docs/proof.md` section 10 gives the exact `F`, the `4h - 3` and `8h - 5` worst cases (tight for `n` a power of two), the copy-on-push identity `F + 2P`, the point-materialization identity and an executable expected-frontier computation, each checked against arena growth or exhaustive enumeration. The lower-bound attempt in model R found a counterexample (edge tags, section 10.6); no optimality claim survives and R was not narrowed. |
